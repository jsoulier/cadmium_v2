...
namespace cadmium::celldevs {
	...
	struct GridCellConfig : public CellConfig<coordinates, S, V> {
	 ...
	 private:
		// replaced the hashmap with a vector so we need a helper for adding entries
		static void setNeighbor(Neighborhood<coordinates, S, V>& neighborhood, const coordinates& cell, const NeighborData<S, V>& data) {
			for (auto& [otherCell, otherData]: neighborhood) {
				if (otherCell == cell) {
					otherData = data;
					return;
				}
			}
			neighborhood.emplace_back(cell, data);
#endif
		}
	};
}
