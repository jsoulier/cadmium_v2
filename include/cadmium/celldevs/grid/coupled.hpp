...
namespace cadmium::celldevs {
	...
	class GridCellDEVSCoupled: public CellDEVSCoupled<coordinates, S, V> {
	 private:
		...
		void loadScenario() {
			CADMIUM_PROFILE_TAG;
			const nlohmann::json& rawScenario = rawConfig.at("scenario");
			auto shape = rawScenario.at("shape").get<coordinates>();
			auto origin = rawScenario.contains("origin") ? rawScenario["origin"].get<coordinates>() : coordinates(0);
	 ...
	 public:
		...
		GridCellDEVSCoupled(const std::string& id, gridCellFactory<S, V> factory, nlohmann::json config):
		  CellDEVSCoupled<coordinates, S, V>(id, std::move(config)), factory(factory) {
			loadScenario();
#endif
		}
